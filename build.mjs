...
const ICONS = {
  sun: `<svg viewBox="0 0 24 24" fill="black"><path d="M12 7c-2.76 0-5 2.24-5 5s2.24 5 5 5 5-2.24 5-5-2.24-5-5-5zM2 13h2c.55 0 1-.45 1-1s-.45-1-1-1H2c-.55 0-1 .45-1 1s.45 1 1 1zm18 0h2c.55 0 1-.45 1-1s-.45-1-1-1h-2c-.55 0-1 .45-1 1s.45 1 1 1zM11 2v2c0 .55.45 1 1 1s1-.45 1-1V2c0-.55-.45-1-1-1s-1 .45-1 1zm0 18v2c0 .55.45 1 1 1s1-.45 1-1v-2c0-.55-.45-1-1-1s-1 .45-1 1zM5.99 4.58a.996.996 0 00-1.41 0 .996.996 0 000 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41L5.99 4.58zm12.37 12.37a.996.996 0 00-1.41 0 .996.996 0 000 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41l-1.06-1.06zm1.06-12.37a.996.996 0 000-1.41.996.996 0 00-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06zM7.05 18.36a.996.996 0 000-1.41.996.996 0 00-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06z"/></svg>`,
  cloud: `<svg viewBox="0 0 24 24" fill="black"><path d="M19.35 10.04A7.49 7.49 0 0012 4C9.11 4 6.6 5.64 5.35 8.04A5.994 5.994 0 000 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96z"/></svg>`,
  rain: `<svg viewBox="0 0 24 24" fill="black"><path d="M19.35 10.04A7.49 7.49 0 0012 4C9.11 4 6.6 5.64 5.35 8.04A5.994 5.994 0 000 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM9 22c0 .55-.45 1-1 1s-1-.45-1-1v-4c0-.55.45-1 1-1s1 .45 1 1v4zm4 0c0 .55-.45 1-1 1s-1-.45-1-1v-4c0-.55.45-1 1-1s1 .45 1 1v4zm4 0c0 .55-.45 1-1 1s-1-.45-1-1v-4c0-.55.45-1 1-1s1 .45 1 1v4z"/></svg>`,
  thunder: `<svg viewBox="0 0 24 24" fill="black"><path d="M19.35 10.04A7.49 7.49 0 0012 4C9.11 4 6.6 5.64 5.35 8.04A5.994 5.994 0 000 14c0 3.31 2.69 6 6 6h3l-1 4h5l-1-4h4c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96z"/></svg>`,
};

const WMO = {
  0: { label: "Céu limpo", icon: "sun" },
  1: { label: "Quase limpo", icon: "sun" },
  2: { label: "Part. Nublado", icon: "cloud" },
  3: { label: "Nublado", icon: "cloud" },
  45: { label: "Nevoeiro", icon: "cloud" },
  51: { label: "Chuviscos", icon: "rain" },
  61: { label: "Chuva", icon: "rain" },
  80: { label: "Aguaceiros", icon: "rain" },
  95: { label: "Trovoada", icon: "thunder" },
};

function getWMO(code) { return WMO[code] || { label: "Instável", icon: "cloud" }; }

async function build() {
  try {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${CONFIG.lat}&longitude=${CONFIG.lon}&hourly=temperature_2m,weathercode&forecast_days=2&timezone=${encodeURIComponent(CONFIG.timezone)}`;
    ...
    const curWMO = getWMO(cur.code);
    
    let hoursListHtml = "";
    for (let i = 1; i <= 4; i++) {
      const j = idx + i;
      const wmo = getWMO(data.hourly.weathercode[j]);
      hoursListHtml += `
      <div class="hour-card">
        <div class="hour-time">${times[j].slice(11, 16)}</div>
        <div class="hour-icon" style="width:55px">${ICONS[wmo.icon]}</div>
        <div class="hour-desc">${wmo.label}</div>
        <div class="hour-temp">${Math.round(data.hourly.temperature_2m[j])}°</div>
      </div>`;
    }
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .topbar-info { text-align: right; font-size: 16px; font-weight: bold; letter-spacing: 1px; }
  ...
  .hour-desc { flex: 1; font-size: 16px; font-weight: bold; text-transform: uppercase; }
  .hour-temp { font-size: 40px; font-weight: 900; }
</style>
</head>
<body>
<div class="topbar">
  <div class="topbar-title">METEOROLOGIA</div>
  <div class="topbar-info">
    <div id="loc-name">${CONFIG.location}</div>
    <div id="top-date">${topDate}</div>
  </div>
</div>
<div class="main" id="main-content">
  <div class="current">
    ...
    <div class="current-desc-box">
      <div class="current-desc">${curWMO.label}</div>
    </div>
  </div>
  <div class="hours" id="hours-list">
    ${hoursListHtml}
  </div>
</div>
</body>
